...
console.log("App iniciado"); // <-- PARA DEPURAR
...
function bottomNav() {
  ...
  items.push(["audit", iconAudit(), "Auditoria"]);
...
function auditRows() {
  ...
  if (state.auditFilters.delegacao !== "TODAS") {
    rows = rows.filter(r => r.worker?.delegacao === state.auditFilters.delegacao);
  }
  if (state.auditFilters.estado !== "TODOS") {
    if (state.auditFilters.estado === "VALIDO") rows = rows.filter(r => r.status === "normal");
    else if (state.auditFilters.estado === "EXPIRADO") rows = rows.filter(r => r.status === "expired");
    else if (state.auditFilters.estado === "AEXPIRAR") rows = rows.filter(r => r.status === "warning");
  }

  return rows.sort((a, b) => (a.validade || "").localeCompare(b.validade || ""));
}
...
function renderAudit() {
  ...
  const expiradosPorWorker = {};
  rows.filter(r => r.status === "expired").forEach(r => {
    const id = r.idTrab;
    expiradosPorWorker[id] = (expiradosPorWorker[id] || 0) + 1;
  });
  const topExpired = Object.entries(expiradosPorWorker)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([id, count]) => {
      const worker = state.data.trabalhadores.find(w => w.id === Number(id));
      return { worker, count };
    })
  ...
  return `
    <section class="section">
      <div class="field-row two">
        <select class="select" data-audit-filter="delegacao">
          ${delegacoes.map(d => `<option ${state.auditFilters.delegacao === d ? "selected" : ""}>${d}</option>`).join("")}
        </select>
        <select class="select" data-audit-filter="estado">
          <option ${state.auditFilters.estado === "TODOS" ? "selected" : ""}>TODOS</option>
          <option ${state.auditFilters.estado === "VALIDO" ? "selected" : ""}>Válidos</option>
          <option ${state.auditFilters.estado === "AEXPIRAR" ? "selected" : ""}>A expirar (≤90 dias)</option>
          <option ${state.auditFilters.estado === "EXPIRADO" ? "selected" : ""}>Expirados</option>
        </select>
      </div>
      ...
      <div style="margin:12px 0">
        <div class="progress" style="height:20px;background:var(--panel-2);border-radius:8px;overflow:hidden">
          <span style="display:block;height:100%;width:${conformidade}%;background:${conformidade >= 80 ? 'var(--ok)' : conformidade >= 60 ? 'var(--warn)' : 'var(--danger)'};text-align:center;color:var(--ink);font-weight:700;line-height:20px;font-size:.85rem">${conformidade}%</span>
        </div>
        <p class="meta">Taxa de conformidade (entregas ativas dentro da validade).</p>
      </div>

      ${topExpired.length ? `
      <div class="section-head"><h2>Trabalhadores com mais EPIs expirados</h2></div>
      <div class="alert-list">
        ${topExpired.map(({ worker, count }) => `
          <div class="alert-card">
            <div><strong>${html(worker.nome)}</strong><span class="meta">${count} EPI(s) expirado(s)</span></div>
            <button class="ghost-btn" data-worker="${worker.id}">Ver ficha</button>
          </div>
        `).join("")}
      </div>
      ` : ""}
...
document.addEventListener("click", ev => {
  ...
  if (modal) { ({ worker: workerModal, delivery: deliveryModal, audit: auditModal, article: articleModal, operadores: operadoresModal })[modal]?.(); return; }
...
document.addEventListener("input", ev => {
  ...
  const auditFilter = ev.target.dataset.auditFilter;
  if (auditFilter) { state.auditFilters[auditFilter] = ev.target.value; render(); return; }
...
document.addEventListener("submit", async ev => {
  ...
  if (kind === "precos") {
    const precos = {};
    state.data.matriz.forEach(epi => {
      const input = form.querySelector(`[name="preco_${epi.nome}"]`);
      if (input) {
        precos[epi.nome] = Number(input.value || 0);
      }
    });
    state.data.precos = precos;
    await saveAll();
