...
// O sistema antigo guardava preços/quantidades em budget.items.
// Nunca apagamos essa informação: usamos-na como fallback para recuperar
// o orçamento já existente e para calcular o custo histórico por funcionário.
function legacyItems(d) {
  const items = d?.budget?.items;
  return items && typeof items === "object" && !Array.isArray(items) ? items : {};
}

function legacyItem(d, name) {
  const item = legacyItems(d)[name];
...
function planningRows(d) {
  ...
  const names = new Set([
    ...Object.keys(legacyItems(d)),
    ...Object.keys(b.planning),
    ...epis(d).map(e => e.nome)
  ]);

  return [...names].filter(Boolean).map(name => {
    const p = b.planning[name] || {};
    const legacyPriceValue = legacyPrice(d, name);
    const legacyQtyValue = legacyQty(d, name);
    const matrizPrice = matrizPrices.get(name) || 0;
    const price = num(p.unitPrice ?? (legacyPriceValue || matrizPrice));
    const qty = num(p.authorizedQty ?? legacyQtyValue);
...
function summary(d) {
  ...
  const planned=plan.reduce((s,r)=>s+r.total,0);
  const spent=ps.reduce((s,p)=>s+totalPurchase(p),0);
  const fam=Object.fromEntries(FAMILIES.map(x=>[x,0]));
  const q=Object.fromEntries(QUARTERS.map(x=>[x,0]));
...
function costView(d) {
  ...
  Object.keys(legacyItems(d)).forEach(name => { if (!prices.get(name)) prices.set(name, legacyPrice(d,name)); });
  const map=new Map();
  deliveries(d).forEach(e=>{
    const id=e.worker_id??e.trabalhador_id??e.workerId??e.trabalhadorId;
    const w=workers(d).find(x=>String(x.id)===String(id));
    const name=w?.nome||e.trabalhador||e.worker_name||e.nomeTrabalhador||e.nome_funcionario||`Trabalhador ${id||""}`;
...
async function render() {
  ...
  try {
    ...
    const d=await load();
    let body=active==="resumo"?summary(d):active==="orcamento"?planningView(d):active==="compras"?purchaseView(d):active==="custo"?costView(d):await archiveView();
...
function bind(root,d) {
  ...
  root.querySelector("[data-save-plan]")?.addEventListener("click",async()=>{
    ...
    root.querySelectorAll("[data-plan]").forEach(i=>{
      ...
      // Mantém também o artigo antigo atualizado, sem apagar outros campos legados.
      if(d.budget.items && d.budget.items[name] && typeof d.budget.items[name]==="object") {
        d.budget.items[name]={...d.budget.items[name],authorizedQty:num(i.value),unitPrice:num(priceInput?.value)};
      }
    });
    await save(d);
  ...
  root.querySelectorAll("[data-archive]").forEach(btn=>btn.addEventListener("click",async()=>{
    const id=btn.dataset.archive,b=budget(d),item=b.purchases.find(x=>x.id===id);if(!item)return;
    if(!confirm(`Arquivar a compra de ${item.product}?`))return;
    await addDoc(collection(firestore(),ARCHIVE),{...item,archivedAt:serverTimestamp()});
...
// Não usamos MutationObserver: o app.js recria o <main> em cada navegação.
