...
async function load() { const s = await getDoc(mainRef()); if (!s.exists()) throw new Error("Não foi encontrado o documento principal de dados."); const d = s.data(); budget(d); return d; }
...
function planningRows(d) { const b=budget(d), prices=new Map(epis(d).map(e=>[e.nome,num(e.preco)])), names=new Set([...Object.keys(b.planning),...epis(d).map(e=>e.nome)]); return [...names].filter(Boolean).map(name=>{const p=b.planning[name]||{}, price=num(p.unitPrice ?? prices.get(name)), qty=num(p.authorizedQty); return {name,price,qty,total:price*qty};}); }
...
function summary(d) { const b=budget(d), plan=planningRows(d), ps=b.purchases, planned=plan.reduce((s,r)=>s+r.total,0), spent=ps.reduce((s,p)=>s+totalPurchase(p),0), fam=Object.fromEntries(FAMILIES.map(x=>[x,0])), q=Object.fromEntries(QUARTERS.map(x=>[x,0])); ps.forEach(p=>{const v=totalPurchase(p);if(fam[p.family]!==undefined)fam[p.family]+=v;if(q[p.quarter]!==undefined)q[p.quarter]+=v;}); return `<div class="budget-view"><div class="budget-kpis"><article><span>Orçamento planeado</span><strong>${euro(planned)}</strong></article><article><span>Gasto realizado</span><strong>${euro(spent)}</strong></article><article><span>Saldo</span><strong>${euro(planned-spent)}</strong></article><article><span>Execução</span><strong>${planned?(spent/planned*100).toFixed(1):"0.0"}%</strong></article></div><div class="budget-grid-2"><section class="budget-card"><h3>Gasto por família</h3>${FAMILIES.map(f=>`<div class="metric-row"><span>${f}</span><strong>${euro(fam[f])}</strong></div>`).join("")}</section><section class="budget-card"><h3>Gasto por trimestre</h3>${QUARTERS.map(x=>`<div class="metric-row"><span>${x}</span><strong>${euro(q[x])}</strong></div>`).join("")}</section></div><section class="budget-card"><h3>Evolução trimestral</h3>${chart(q)}</section></div>`; }
function planningView(d) { const rows=planningRows(d); return `<div class="budget-view"><div class="budget-help"><strong>Orçamento:</strong> quantidade autorizada × preço unitário. O gasto real só é registado em <strong>Compras</strong>.</div><section class="budget-card"><div class="section-head"><div><h3>Planeamento</h3><p class="muted">Define apenas a quantidade autorizada a comprar.</p></div><strong>${euro(rows.reduce((s,r)=>s+r.total,0))}</strong></div><div class="table-wrap"><table class="budget-table"><thead><tr><th>EPI</th><th>Preço</th><th>Qtd. autorizada</th><th>Orçamento</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${euro(r.price)}</td><td><input type="number" min="0" step="1" data-plan="${esc(r.name)}" value="${r.qty||""}"></td><td>${euro(r.total)}</td></tr>`).join("")}</tbody></table></div><button type="button" class="primary" data-save-plan>Guardar planeamento</button></section></div>`; }
function purchaseView(d) { const b=budget(d), options=epis(d).map(e=>`<option value="${esc(e.nome)}">${esc(e.nome)}</option>`).join(""), rows=b.purchases.slice().sort((a,c)=>String(c.date).localeCompare(String(a.date))); return `<div class="budget-view"><section class="budget-card"><h3>Registar compra</h3><div class="form-grid"><label>Família<select id="bf"><option>EPI</option><option>Equipamento</option><option>Ambiente</option></select></label><label>Trimestre<select id="bq">${QUARTERS.map(q=>`<option>${q}</option>`).join("")}</select></label><label>Data<input id="bd" type="date" value="${today()}"></label><label id="epi-field">EPI<select id="be"><option value="">Selecionar EPI</option>${options}</select></label><label id="product-field" class="wide">Produto<input id="bp" placeholder="Produto / equipamento / serviço"></label><label>Quantidade<input id="bqty" type="number" min="0" step="1"></label><label>Preço unitário (€)<input id="bprice" type="number" min="0" step="0.01"></label><label>Fornecedor<input id="bsupplier"></label><label>N.º fatura<input id="binvoice"></label></div><button type="button" class="primary" data-add-purchase>Registar compra</button></section><section class="budget-card"><div class="section-head"><div><h3>Compras registadas</h3><p class="muted">Cada compra é registada uma única vez. O Resumo e o trimestre são atualizados automaticamente.</p></div><strong>${euro(rows.reduce((s,p)=>s+totalPurchase(p),0))}</strong></div><div class="table-wrap"><table class="budget-table"><thead><tr><th>Data</th><th>Família</th><th>Trimestre</th><th>Produto</th><th>Qtd.</th><th>Preço</th><th>Total</th><th>Fornecedor</th><th></th></tr></thead><tbody>${rows.length?rows.map(p=>`<tr><td>${esc(p.date)}</td><td>${esc(p.family)}</td><td>${esc(p.quarter)}</td><td>${esc(p.product)}</td><td>${num(p.quantity)}</td><td>${euro(p.unitPrice)}</td><td>${euro(totalPurchase(p))}</td><td>${esc(p.supplier)}</td><td><button type="button" class="danger-link" data-archive="${esc(p.id)}">Arquivar</button></td></tr>`).join(""):`<tr><td colspan="9">Ainda não existem compras registadas.</td></tr>`}</tbody></table></div></section></div>`; }
function costView(d) { const prices=new Map(epis(d).map(e=>[e.nome,num(e.preco)])), map=new Map(); deliveries(d).forEach(e=>{const id=e.worker_id??e.trabalhador_id??e.workerId,w=workers(d).find(x=>String(x.id)===String(id)),name=w?.nome||e.trabalhador||e.worker_name||`Trabalhador ${id||""}`,product=e.epi_type||e.epi||e.nomeEpi||e.nome||"EPI",qty=num(e.qtd??e.quantidade??e.qty)||1,unit=prices.get(product)??num(e.preco??e.unitPrice),key=String(id||name);if(!map.has(key))map.set(key,{name,funcao:w?.funcao||"",delegacao:w?.delegacao||"",qty:0,cost:0});const r=map.get(key);r.qty+=qty;r.cost+=qty*unit;}); const rows=[...map.values()].sort((a,b)=>b.cost-a.cost); return `<div class="budget-view"><section class="budget-card"><h3>Custo por Funcionário</h3><p class="muted">Calculado a partir das entregas reais. O gasto trimestral não é tratado neste módulo.</p><div class="table-wrap"><table class="budget-table"><thead><tr><th>Funcionário</th><th>Função</th><th>Delegação</th><th>Qtd. EPI</th><th>Custo</th></tr></thead><tbody>${rows.length?rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.funcao)}</td><td>${esc(r.delegacao)}</td><td>${r.qty}</td><td>${euro(r.cost)}</td></tr>`).join(""):`<tr><td colspan="5">Ainda não existem entregas suficientes.</td></tr>`}</tbody></table></div></section></div>`; }
async function archiveView() { const s=await getDocs(collection(firestore(),ARCHIVE)),rows=s.docs.map(x=>({id:x.id,...x.data()})).sort((a,b)=>String(b.date).localeCompare(String(a.date))); return `<div class="budget-view"><section class="budget-card"><div class="section-head"><div><h3>Arquivo</h3><p class="muted">Histórico retirado da vista principal, sem apagar os dados.</p></div><strong>${rows.length} registos</strong></div><div class="table-wrap"><table class="budget-table"><thead><tr><th>Data</th><th>Família</th><th>Trimestre</th><th>Produto</th><th>Qtd.</th><th>Total</th><th></th></tr></thead><tbody>${rows.length?rows.map(r=>`<tr><td>${esc(r.date)}</td><td>${esc(r.family)}</td><td>${esc(r.quarter)}</td><td>${esc(r.product)}</td><td>${num(r.quantity)}</td><td>${euro(totalPurchase(r))}</td><td><button type="button" class="danger-link" data-delete-archive="${esc(r.id)}">Apagar</button></td></tr>`).join(""):`<tr><td colspan="7">O arquivo está vazio.</td></tr>`}</tbody></table></div></section></div>`; }

async function render() {
  const main=page(); if(!main||busy)return; busy=true;
  try { let root=main.querySelector(".budget-management-root"); if(!root){root=document.createElement("section");root.className="budget-management-root";main.insertBefore(root,main.firstChild);} [...main.children].forEach(c=>{if(c!==root&&c.classList.contains("section"))c.style.display="none";}); root.innerHTML=`${tabs()}<div class="budget-loading">A carregar...</div>`; const d=await load(); let body=active==="resumo"?summary(d):active==="orcamento"?planningView(d):active==="compras"?purchaseView(d):active==="custo"?costView(d):await archiveView(); root.innerHTML=`${tabs()}${body}`; bind(root,d); lastMain=main;
  } catch(e) { const r=main.querySelector(".budget-management-root"); if(r)r.innerHTML=`${tabs()}<div class="budget-error"><strong>Não foi possível carregar o módulo.</strong><br>${esc(e.message||e)}</div>`; } finally {busy=false;}
}
function bind(root,d) {
  ...
  function sync(){const isEpi=family?.value==="EPI";if(epiField)epiField.style.display=isEpi?"grid":"none";if(productField)productField.style.display=isEpi?"none":"grid";if(epi)epi.disabled=!isEpi;if(isEpi&&epi?.value){const e=epis(d).find(x=>x.nome===epi.value);if(e)price.value=num(e.preco)||"";}}
  family?.addEventListener("change",sync);epi?.addEventListener("change",sync);sync();
  root.querySelector("[data-save-plan]")?.addEventListener("click",async()=>{const b=budget(d);root.querySelectorAll("[data-plan]").forEach(i=>{const name=i.dataset.plan,old=b.planning[name]||{},e=epis(d).find(x=>x.nome===name);b.planning[name]={...old,family:"EPI",authorizedQty:num(i.value),unitPrice:num(old.unitPrice??e?.preco)};});await save(d);await render();});
  root.querySelector("[data-add-purchase]")?.addEventListener("click",async()=>{const fam=family.value,date=root.querySelector("#bd").value||today(),q=root.querySelector("#bq").value||quarterOf(date),productName=(fam==="EPI"?epi.value:product.value).trim(),qty=num(root.querySelector("#bqty").value),unit=num(price.value);if(!productName||qty<=0||unit<0){alert("Preenche o produto, quantidade e preço unitário.");return;}const b=budget(d);b.purchases.push({id:`buy_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,family:fam,quarter:q,date,product:productName,quantity:qty,unitPrice:unit,supplier:root.querySelector("#bsupplier").value.trim(),invoice:root.querySelector("#binvoice").value.trim(),createdAt:new Date().toISOString()});await save(d);await render();});
...
// Não usamos MutationObserver: o app.js recria o <main> em cada navegação e um observer
// sobre o body provocaria ciclos de renderização. O polling só inicializa o módulo quando
// a página Orçamento aparece e volta a ligá-lo quando o utilizador regressa a essa página.
