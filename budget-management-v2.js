...
async function save(d) {
  budget(d);
  await setDoc(mainRef(), { budget: d.budget }, { merge: true });
}
...
function planningRows(d) {
  ...
  const names = new Set([...Object.keys(b.planning), ...epis(d).map(e => e.nome)]);
...
function tabs() {
  return `<nav class="budget-management-tabs" role="tablist" aria-label="Módulos do orçamento">${TABS.map(([id,label])=>`<button type="button" role="tab" class="budget-management-tab ${active===id?"active":""}" data-tab="${id}" aria-selected="${active===id}">${label}</button>`).join("")}</nav>`;
}

function chart(values) {
  const max = Math.max(1, ...QUARTERS.map(q => values[q] || 0));
  return `<div class="chart-bars">${QUARTERS.map(q=>{const v=values[q]||0,h=Math.max(4,v/max*150);return `<div class="chart-col"><span>${euro(v)}</span><div class="chart-bar" style="height:${h}px"></div><b>${q}</b></div>`;}).join("")}</div>`;
}

function summary(d) {
  const b=budget(d), plan=planningRows(d), ps=b.purchases, planned=plan.reduce((s,r)=>s+r.total,0), spent=ps.reduce((s,p)=>s+totalPurchase(p),0), fam=Object.fromEntries(FAMILIES.map(x=>[x,0])), q=Object.fromEntries(QUARTERS.map(x=>[x,0]));
...
function costView(d) {
  const prices=new Map(epis(d).map(e=>[e.nome,num(e.preco)])), map=new Map();
  deliveries(d).forEach(e=>{const id=e.worker_id??e.trabalhador_id??e.workerId,w=workers(d).find(x=>String(x.id)===String(id)),name=w?.nome||e.trabalhador||e.worker_name||`Trabalhador ${id||""}`,product=e.epi_type||e.epi||e.nomeEpi||e.nome||"EPI",qty=num(e.qtd??e.quantidade??e.qty)||1,unit=prices.get(product)??num(e.preco??e.unitPrice),key=String(id||name);if(!map.has(key))map.set(key,{name,funcao:w?.funcao||"",delegacao:w?.delegacao||"",qty:0,cost:0});const r=map.get(key);r.qty+=qty;r.cost+=qty*unit;});
  const rows=[...map.values()].sort((a,b)=>b.cost-a.cost);
  return `<div class="budget-view"><section class="budget-card"><h3>Custo por Funcionário</h3><p class="muted">Calculado a partir das entregas reais. O gasto trimestral não é tratado neste módulo.</p><div class="table-wrap"><table class="budget-table"><thead><tr><th>Funcionário</th><th>Função</th><th>Delegação</th><th>Qtd. EPI</th><th>Custo</th></tr></thead><tbody>${rows.length?rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${esc(r.funcao)}</td><td>${esc(r.delegacao)}</td><td>${r.qty}</td><td>${euro(r.cost)}</td></tr>`).join(""):`<tr><td colspan="5">Ainda não existem entregas suficientes.</td></tr>`}</tbody></table></div></section></div>`;
}
...
function bind(root,d) {
  ...
  root.querySelectorAll("[data-plan-price],[data-plan]").forEach(input=>input.addEventListener("input",()=>{
    const name=input.dataset.plan||input.dataset.planPrice;
    const priceInput=root.querySelector(`[data-plan-price="${CSS.escape(name)}"]`);
