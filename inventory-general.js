...
function render(){
  ...
  root.querySelector('[data-ig-close]')?.addEventListener('click',()=>{state.open=false;root.innerHTML=''});root.querySelector('[data-ig-overlay]')?.addEventListener('click',e=>{if(e.target===e.currentTarget){state.open=false;root.innerHTML=''}});
  root.querySelectorAll('[data-ig-family]').forEach(b=>b.addEventListener('click',()=>{state.family=b.dataset.igFamily;render();}));root.querySelector('[data-ig-add]')?.addEventListener('click',addItem);root.querySelectorAll('[data-ig-toggle]').forEach(b=>b.addEventListener('click',()=>toggle(Number(b.dataset.igToggle))));root.querySelectorAll('[data-ig-models]').forEach(b=>b.addEventListener('click',()=>openModels(Number(b.dataset.igModels))));
}
