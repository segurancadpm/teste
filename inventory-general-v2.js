...
function items(f=family){ ensureShape(); return data.budget.inventoryCatalog.families[f] || []; }
function active(f){ return items(f).filter(x => x.active !== false); }
function models(epi){ ensureShape(); return Array.isArray(data.epiModels[epi]) ? data.epiModels[epi] : []; }
...
function catalogExisting(){
  ...
  FAMILIES.forEach(f => (Array.isArray(old[f]) ? old[f] : []).forEach(x => add(f, x?.name ?? x?.nome, "legacy")));
  ...
  data.budget.inventoryCatalog.version = 6;
...
async function save(){ ensureShape(); syncBudget(); await setDoc(ref(), {budget:data.budget, matriz:data.matriz, stocks:data.stocks, epiModels:data.epiModels}, {merge:true}); }
...
function closeManager(){ open=false; const root=document.getElementById("modal-root"); if(root) root.innerHTML=""; }
...
function render(){
  ...
  root.querySelector('[data-ig-close]')?.addEventListener('click',closeManager);
  root.querySelector('[data-ig-overlay]')?.addEventListener('click',e=>{if(e.target===e.currentTarget)closeManager();});
  root.querySelectorAll('[data-family]').forEach(b=>b.addEventListener('click',()=>{family=b.dataset.family;render();}));
...
async function addItem(){
  ...
  try { await save(); filterOperations(); render(); } catch(e) { alert(`Não foi possível adicionar o artigo.\n\n${e.message||e}`); }
}
async function toggleItem(i){ ensureShape(); const x=items()[i]; if(!x)return; const old=x.active!==false; x.active=!old; try{await save();filterOperations();render();}catch(e){x.active=old;alert(`Não foi possível alterar o artigo.\n\n${e.message||e}`);} }
...
async function openManager(){if(open)return;open=true;try{await load();await save();filterOperations();render();}catch(e){open=false;console.error("Inventário Geral:",e);alert(`Não foi possível abrir o Inventário Geral.\n\n${e.message||e}`);}}
...
function start(){new MutationObserver(()=>{filterOperations();inject();}).observe(document.body,{childList:true,subtree:true});setTimeout(async()=>{try{await load();await save();filterOperations();inject();}catch(e){console.warn('Inventário Geral',e);}},500);}
start();
