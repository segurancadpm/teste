...
function render(){
  ...
  root.querySelectorAll('[data-move]').forEach(sel=>sel.addEventListener('change',()=>moveItem(Number(sel.dataset.move),sel.value))); root.querySelectorAll('[data-toggle]').forEach(b=>b.addEventListener('click',()=>toggleItem(Number(b.dataset.toggle)))); root.querySelectorAll('[data-models]').forEach(b=>b.addEventListener('click',()=>showModels(Number(b.dataset.models))));
}
