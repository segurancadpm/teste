...
import { getFirestore, doc, getDoc, setDoc, getDocs, collection, query, where, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
function render(){
  ...
  root.querySelectorAll('[data-move]').forEach(sel=>sel.addEventListener('change',()=>moveItem(Number(sel.dataset.move),sel.value))); root.querySelectorAll('[data-delete-item]').forEach(b=>b.addEventListener('click',()=>deleteItem(Number(b.dataset.deleteItem)))); root.querySelectorAll('[data-models]').forEach(b=>b.addEventListener('click',()=>showModels(Number(b.dataset.models))));
}
...
async function deleteItem(index){
  ensure(); const list=items(); const item=list[index]; if(!item)return; const name=String(item.name||'').trim(); if(!name)return;
  const events=Array.isArray(state.data.eventos)?state.data.eventos.filter(e=>norm(e?.epi)===norm(name)):[];
  let stockTotal=0;
  Object.values(state.data.stocks||{}).forEach(w=>{const r=w?.[name]; if(typeof r==='number')stockTotal+=num(r); else {stockTotal+=num(r?.loose);Object.values(r?.sizes||{}).forEach(v=>stockTotal+=num(v));}});
  let deliveryCount=0;
  if(state.family==='EPI'){
    try{deliveryCount=(await getDocs(query(collection(db(),'deliveries'),where('epi_type','==',name)))).size;}catch(e){console.warn('Não foi possível verificar deliveries:',e);}
  }
  if(stockTotal||events.length||deliveryCount){
    alert(`Não posso apagar "${name}" porque já existem dados associados.\n\nStock: ${stockTotal}\nEventos: ${events.length}\nEntregas: ${deliveryCount}\n\nO artigo deve permanecer no histórico.`); return;
  }
  if(!confirm(`Apagar definitivamente "${name}"?\n\nO artigo não tem stock nem histórico associado. Esta ação não pode ser desfeita.`))return;
  try{
    await runTransaction(db(),async tx=>{
      const snap=await tx.get(ref()); const x=snap.data()||{}; const budget={...(x.budget||{})}; const inv={...(budget.inventoryCatalog||{})}; const fams={...(inv.families||{})};
      Object.keys(fams).forEach(f=>{if(Array.isArray(fams[f]))fams[f]=fams[f].filter(i=>norm(i?.name)!==norm(name));}); inv.families=fams;
      if(budget.items){const copy={...budget.items};delete copy[name];budget.items=copy;}
      if(budget.management?.planning){const p={...budget.management.planning};delete p[name];budget.management={...(budget.management||{}),planning:p};}
      ...
      const stocks={...(x.stocks||{})};Object.keys(stocks).forEach(w=>{const copy={...(stocks[w]||{})};delete copy[name];stocks[w]=copy;});
      tx.set(ref(),{budget,matriz,epiModels,stocks},{merge:true});
    });
    await load(); alert(`"${name}" foi apagado do Inventário Geral.`); render();
